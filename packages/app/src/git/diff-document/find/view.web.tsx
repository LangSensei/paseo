...
import { PaneFind, type PaneFindHandle } from "./widget";
...
export function DiffFindOverlay({
  ...
  else if (snapshot.query && snapshot.matches.length === 0)
    status = t("diffFindControls.noMatches");
  else if (snapshot.matches.length)
    status = t("diffFindControls.position", { current: snapshot.current + 1, total });
