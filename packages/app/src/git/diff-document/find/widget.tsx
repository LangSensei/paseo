// UI copied from Paseo PR #4589 (db8c22e1); the shared-4589 variant imports @/pane-find.
import {
  forwardRef,
  useCallback,
  useLayoutEffect,
...
export const PaneFind = forwardRef<PaneFindHandle, PaneFindProps>(function PaneFind(
  ...
  const matchCount = useMemo(
    () => (
      <Text
        style={styles.status}
        role="status"
        accessibilityLabel={t("diffFindControls.matches")}
  ...
  return (
    <View
      style={styles.widget}
      accessibilityLabel={t("diffFindControls.title")}
      {...(isWeb
        ? {
    ...
    >
      {replace ? (
        <View style={gutterStyle}>
          <ToolbarButton
            label={t("diffFindControls.toggleReplace")}
      ...
      <View style={styles.rows}>
        <View style={styles.row}>
          <FindField
            ref={input}
            autoFocus
            label={t("diffFindControls.placeholder")}
            initialValue={query}
            height={rowHeight}
          ...
          />
          <ToolbarButton
            label={t("diffFindControls.previous")}
            compact={isCompact}
          ...
          <ToolbarButton label={t("diffFindControls.close")} compact={isCompact} onPress={onClose}>
        ...
        {replace && replaceExpanded ? (
          <View style={styles.row}>
            <FindField
              ...
              label={t("diffFindControls.replaceWith")}
